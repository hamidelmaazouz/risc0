...
use super::{align_bytes_to_words,  Result, as_words_padded};
use core::ops::{Index,IndexMut};

pub struct Alloc<'a> {
    buf_left: &'a mut Option<&'a mut [u32]>,
    base: usize,
}
...
impl<'a> Alloc<'a> {
    ...
    fn alloc(&mut self, words: usize) -> AllocBuf<'a> {
        let buf_left = std::mem::replace(self.buf_left, None).unwrap();
        assert!(buf_left.len() >= words);
        let (new_buf, rest) = buf_left.split_at_mut(words);
        (*self.buf_left) = Some(rest);
    ...
    T: Serialize,
{
    ...
    buf.resize(tot_len, 0);

    let mut alloc_buf: Option<&mut [u32]> = Some(buf.as_mut_slice());

    let mut alloc = Alloc {
        buf_left: &mut alloc_buf,
        base: 0,
    };
...
pub trait Serialize {
    ...
    fn fixed_len() -> usize { Self::FIXED_WORDS }
...
impl Serialize for std::string::String {
    ...
    fn fill(&self, buf: &mut AllocBuf, a: &mut Alloc) -> Result<()> {
        ...
        for (w, val) in std::iter::zip(str_data.buf.iter_mut(), as_words_padded(self.as_bytes().into_iter().cloned())) {
            *w = val;
        }
